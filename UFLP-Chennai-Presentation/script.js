  ...
  Object.entries(warehouses).forEach(([w, data]) => {
    ...
    whContainer.innerHTML += `
      <label class="wh-checkbox-label" id="lbl-${w}" for="cb-${w}">
        ...
        <span class="wh-cost-chip">₹${fixedCost[w].toLocaleString()}</span>
      </label>
    `;
    ...
    sliderContainer.innerHTML += `
      <div class="slider-row">
        <div class="slider-top">
          ...
          <span class="slider-value" id="val-${w}" style="color:${data.color}">${fixedCost[w].toLocaleString()}</span>
        </div>
        <input type="range" id="slider-${w}" min="0" max="80" step="1" value="${fixedCost[w]}" style="accent-color:${data.color}" />
        <div class="slider-meta"><span>0</span><span>Fixed Cost (₹)</span><span>80</span></div>
      </div>
    `;
  });
}
...
const map = L.map('map', { zoomControl: true }).setView([13.00, 80.10], 11);
...
function initMapMarkers() {
  ...
  Object.entries(warehouses).forEach(([id, w]) => {
    warehouseMarkers[id] = L.marker(w.coords, { icon: makeWhIcon(w.color) })
      .addTo(map)
      .bindPopup(`<b>${id} — ${w.name}</b><br><span style="color:#94a3b8">${w.area}</span><br>Fixed cost: <b id="popup-cost-${id}">${fixedCost[id].toLocaleString()}</b>`);
  });
...
function updateDashboard(openWh, skipTableHighlight = false) {
  ...
  clearLines();
  if (openWh.length) {
    Object.entries(assignments).forEach(([z, a]) => {
      if (!a.w) return;
...
function renderAssignments(assignments, openWh) {
  ...
  wrap.innerHTML = Object.entries(zones).map(([z, zd]) => {
    const a = assignments[z];
    const wBadge = a?.w
      ? `<span class="kpi-wh-badge none" style="background:rgba(255,255,255,0.1);color:${warehouses[a.w].color};border-color:${warehouses[a.w].color}">${a.w}</span>`
      : `<span class="kpi-wh-badge none">—</span>`;
    const cost = a?.cost ?? '—';
    return `<div class="kpi-assign-row">
      <span class="kpi-zone">${z} ${zd.name}</span>
      <span class="kpi-arrow">→</span>
      ${wBadge}
      <span class="kpi-cost">₹${cost.toLocaleString()}</span>
    </div>`;
  }).join('');
}
...
function renderCaseTable(activeWh = []) {
  ...
  // If there are many cases (e.g. 255), limit the render visually or slice it if needed.
  // Actually 255 rows is fine for modern browsers, but we will sort them by cost.
  ...
  tbody.innerHTML = sortedCases.map((c, i) => {
    ...
    // original index for simulating case
    ...
    return `<tr class="${rowClass}" data-idx="${originalIdx}" onclick="simulateCase(${originalIdx})">
      ...
      <td class="cost-num" style="color:var(--accent-cyan)">${c.transport.toLocaleString()}</td>
      <td class="cost-num" style="color:var(--accent-amber)">${c.fixed.toLocaleString()}</td>
      <td class="cost-num" style="color:var(--accent-green);font-weight:700">${c.total.toLocaleString()}</td>
      <td>${statusHtml}</td>
    </tr>`;
  }).join('');
}
...
function renderChart(activeWh = []) {
  ...
  // For 255 cases, a bar chart is unreadable. Let's slice the top 50 cases (sorted by cost)
    ...
    options: {
      ...
      plugins: {
        ...
        tooltip: {
          ...
          callbacks: { footer: (items) => `Total: ${sortedCases[items[0].dataIndex].total.toLocaleString()}` }
        }
      },
      scales: {
        x: { stacked: true, grid: { display: false }, ticks: { display: false } }, // Hiding x ticks for dense chart
        y: { stacked: true, grid: { color: 'rgba(255,255,255,0.06)' }, ticks: { color: '#64748b', font: { family: 'Inter', size: 11 } }, title: { display: true, text: 'Cost (₹)', color: '#475569', font: { size: 11 } } }
      }
    }
  });
}
...
function initSliders() {
  Object.keys(warehouses).forEach(w => {
    ...
    slider.addEventListener('input', () => {
      ...
      valEl.textContent   = parseInt(slider.value).toLocaleString();

      const pp = document.getElementById(`popup-cost-${w}`);
      if (pp) pp.textContent = parseInt(slider.value).toLocaleString();
      
      // Update UI label cost chip
      const lblCost = document.querySelector(`#lbl-${w} .wh-cost-chip`);
      if(lblCost) lblCost.textContent = `₹${parseInt(slider.value).toLocaleString()}`;
...
function renderSensitivityAlert() {
  ...
  el.querySelector('.optimal-alert-title').textContent =
    changed
      ...
      : `✅ Optimal: ${opt.w.join(' + ')} (${opt.name}) — Total Cost: ${opt.total.toLocaleString()}`;
  ...
  el.innerHTML = `
    <div class="why-headline">
      ...
      <div>
        ...
        <div style="font-size:11px;color:var(--text-muted);margin-top:2px">Total Cost: <b style="color:var(--accent-green)">${opt.total.toLocaleString()}</b> | Warehouses: ${opt.w.map(w=>`<span class="wh-tag none" style="color:${warehouses[w].color};border:1px solid ${warehouses[w].color}40">${w}</span>`).join('')}</div>
      </div>
    </div>
    <ul class="why-points">
...
async function runBranchAndBound() {
  ...
  for (let i = 0; i < globalAllCases.length; i++) {
    ...
    // For 255 permutations, if turbo mode, skip console rendering for pruned ones occasionally
    ...
    if(renderNode) {
      ...
      log(`  Fixed Binding = ${c.fixed.toLocaleString()}`);
      log(`  Transport   = ${c.transport.toLocaleString()}`);
    }

    const lowerBound = c.transport; 
    
    if(renderNode) {
      log(`  Lower Bound ≈ ${lowerBound.toLocaleString()}`);
      await sleep(speed * 0.4); if (bbAbort) break;
    }

    if (lowerBound > bestCost) {
      if(renderNode) log(`  <span class="log-prune">✗ PRUNED (${lowerBound.toLocaleString()} > best ${bestCost.toLocaleString()})</span>\n`);
      if(speed > 100) await sleep(speed * 0.2);
      continue;
    }

    if(renderNode) {
      log(`  Actual Cost = <b>${c.total.toLocaleString()}</b>`);
      await sleep(speed * 0.4); if (bbAbort) break;
    }
    ...
    if (c.total < bestCost) {
      bestCost = c.total;
      bestIdx  = i;
      log(`  <span class="log-best">✓ NEW INCUMBENT → ${c.label} = ${bestCost.toLocaleString()}</span>\n`);
      simulateCase(i);
    } else {
      if(renderNode) log(`  (Bound discarded: ${c.total.toLocaleString()} > ${bestCost.toLocaleString()})\n`);
    }
...
function exportCsv() {
  ...
  link.setAttribute("download", "uflp_pan_india_cases.csv"); // updated filename
